...
$$
Dette er illustreret i @fig-graf_kontinuert.
```{r}
#| message: false
#| fig-cap: Grafen for en kontinuert funktion.
#| label: fig-graf_kontinuert
library(ggplot2)
#library(shiny)
f = function(x) 0.02*(x + 2)*(x - 3)*(x - 5)
fig <- ggplot() +
  xlim(c(-4, 8)) +
  geom_function(fun = f, size=1) + theme_minimal()  +   ylab("y") + xlab("x") + 
  geom_segment(aes(x=x1, y=y1, xend=x2, yend=y2), data = data.frame(x1=c(-4,0), x2 = c(8,0), y1 = c(0, -3.2), y2 = c(0, 3.2)), arrow = arrow(length = unit(0.2,"cm"), type = "closed")) + coord_cartesian(xlim = c(-4, 8), ylim = c(-3,3)) + geom_point(aes(x=1,y=f(1)),colour="black",size=4)
fig

# Making slider - VIRKER IKKE!
#renderPlot({
# ggplot() +
#  xlim(c(-4, 8)) +
#  geom_function(fun = f, size=1) + geom_point(aes(x=1,y=f(1)),colour="black",size=4) + geom_point(aes(x=1 + input$h,y=f(1+input$h)),colour="black",size=4)
# })
#inputPanel(
# sliderInput("h", label = "Tilvækst h:",
# min = 0, max = 1.5, value = 0.50)
#)
...
```{r}
#| message: false
#| fig-cap: Illustration af middelværdisætningen. Her har tangenten i $(1,f(1))$ (den grønne linje) samme hældning som sekanten gennem $(0,f(0))$ og $(4,f(4))$ (den blå linje).
#| label: fig-middelvaerdisaetningen_test
# HELP! Det kan vist gøres med ggplot - men det er da noget bøvl!
#library(plotly)
#f <- function(x){sqrt(x)}
#x <- 0:4
#y <- f(x)
...
```{r}
#| message: false
#| fig-cap: Grafen for funktionen med forskrift $f\left( x,y \right) = \left( \left( x - 5 \right)^{2} + 3 \right) \cdot \left( 5 + \left( y - 10 \right)^{2} \right) + 30$.
#| label: fig-graf
library(plotly)
f <- function(x,y){((x-5)^2+3)*(5+(y-10)^2)+30}
x <- -10:20
y <- -5:25
z <- outer(x, y, f)
...
## ellers virker colorscale ikke
## HELP! Jeg vil gerne have vist snitkurver
fig <- fig |> add_surface(cmin=30, cmax=250)
fig
```
